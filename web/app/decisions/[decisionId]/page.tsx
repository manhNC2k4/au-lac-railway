...
export default function DecisionDetailPage({params}:{params:{decisionId:string}}){
 const decisionId=decodeURIComponent(params.decisionId);
 const api=getApi();const query=useQuery({queryKey:qk.decision(decisionId),queryFn:()=>api.getDecision(decisionId)});if(query.isPending)return <PageSkeleton/>;if(query.isError)return <ErrorState error={query.error} onRetry={()=>query.refetch()}/>;const d=query.data;const accept=d.action==="ACCEPT";const bids=Object.entries(d.bid_price_breakdown??{});const rulesFired=d.audit_timeline?.rules_fired??[];
 ...
 <div className={`rounded-lg border p-4 ${accept?"border-success/40 bg-success-soft":"border-danger/40 bg-danger-soft"}`}><div className="flex flex-wrap items-center gap-3"><StatusBadge status={d.action}/><p className="flex-1 text-sm text-ink">{d.audit_timeline?.explanation??"Quyết định nạp từ dữ liệu mock — không có diễn giải luật giá theo thời gian thực."}</p><code className="text-xs text-muted">{d.input_hash}</code></div></div>
...
function Version({label,value}:{label:string;value:number|undefined}){return <div className="rounded-lg bg-surface p-3"><p className="text-xs text-muted">{label}</p><strong className="text-lg text-ink">{value!=null?`v${value}`:"—"}</strong></div>}
