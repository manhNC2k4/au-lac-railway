...
import { GOLDEN, STATIONS, segmentStations } from "@/lib/constants";
...
export default function OpsOverviewPage(){
 const api=getApi(); const overview=useQuery({queryKey:qk.overview(GOLDEN.serviceRunId),queryFn:()=>api.getOverview(GOLDEN.serviceRunId)}); const analytics=useQuery({queryKey:qk.analytics(GOLDEN.serviceRunId),queryFn:()=>api.getAnalytics(GOLDEN.serviceRunId)});
 ...
 return <div className="space-y-4">
  <section className="relative min-h-[176px] overflow-hidden rounded-lg border border-line bg-[#dceeff] shadow-card"><div className="relative z-10 max-w-[760px] p-6"><span className="rounded-lg bg-primary px-3 py-1.5 text-sm font-bold text-white">{GOLDEN.serviceRunId}</span><h1 className="mt-4 text-[25px] font-bold text-ink">Hà Nội → Sài Gòn</h1><p className="mt-2 text-sm text-muted">Ngày chạy {GOLDEN.serviceDate} · {GOLDEN.seatClass}</p></div><TrainArt className="absolute -bottom-1 right-3 hidden w-[430px] md:block"/></section>
