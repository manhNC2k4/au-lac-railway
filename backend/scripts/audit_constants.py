...
# 4 = số chữ số thập phân làm tròn chuẩn của repo (round(x, 4) cho score/ratio/divergence).
WHITELIST = {0, 1, -1, 2, 3, 4, 100, 1000,
             200, 201, 202, 400, 403, 404, 409, 410, 422, 500, 503,
             7, 8, 40}
...
def _index_exempt_ids(tree: ast.Module) -> set[int]:
    """Literal dùng làm CHỈ SỐ truy cập (`row[4]`, `.hex[:12]`) không phải hằng số
    nghiệp vụ (không đo lường/hiệu chỉnh gì) — miễn khai nguồn, khác hẳn category mà
    sweep này nhắm tới (số bịa/chọn tay trong tính toán)."""
    exempt: set[int] = set()
    for node in ast.walk(tree):
...
def audit_file(path: Path) -> list[tuple[int, str]]:
    ...
    index_exempt = _index_exempt_ids(tree)
